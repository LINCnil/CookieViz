...
function get_json(max_date, domain) {
    ...
    $.ajax({
        async: false,
        dataType: "json",
        url: "json.php?max_date=" + max_date + "&domain=" + domain,
        success: function (html) {
            res = html;
        },
        error: function (response) {
            generate("top", "error:" + response.responseText, "error");
        }
    });
    return res;
}
...
function reset_graph() {
    ...
    $.ajax({
        ...
        error: function (response) {
            ...
            //generate("topLeft", url, "error");
            //generate("error: " + response.responseText+" "+url);
        }
    });
...
function delete_point() {
    location.href = "index.php?max_date&domain=" + $("#context_menu").attr("domain");
}
function draw_points(el, w, h) {
    ...
    this.addLink = function (d, map_nodes_bis) {
        ...
        var redraw = node.filter(function (d, $to_redraw) {
            if (typeof to_redraw[d.name] != 'undefined') {
                return d.id;
            }
        });
        redraw.remove();
        links.push({
            "source": map_nodes_bis[d.source],
            "target": map_nodes_bis[d.target],
            "cookie": d.cookie,
            "id": cpt_link
        });
        cpt_link++;
    ...
    var update = function () {
        ...
        var linkEnter = link.enter().insert("line")
            ...
            .style("stroke", function (d) {
                return fill[d.cookie];
            });
        link.exit().remove();

        var grow = node.filter(function (d, $to_grow) {
            if (typeof to_grow[d.name] != 'undefined') {
                return d.id;
            }
        });
        grow.selectAll("circle")
            .attr("r", function (d, $to_grow) {
                return to_grow[d.name];
            });
